...
describe("🚩 Challenge 0: 🎟 Simple NFT Example 🤓", function () {
  ...
  describe("SimpleNFT", function () {
    ...
    if (contractAddress) {
      // For the autograder.
      contractArtifact = `contracts/download-${contractAddress}.sol:SimpleNFT`;
    } else {
      contractArtifact = "contracts/SimpleNFT.sol:SimpleNFT";
    }

    it("Should deploy the contract", async function () {
      const SimpleNFT = await ethers.getContractFactory(contractArtifact);
      myContract = await SimpleNFT.deploy();
